...
class InvitationViewSet(GenericViewSet):
    ...
    def send_invitation(self, request, pk=None):
        ...
        already_invited = models.Invitation.objects.filter(
            sender=current_user, receiver=invited_user
        ).exists()

        if already_invited:
            message = {"detail": "You already invited this user"}
            return Response(
                message,
        ...
        if not current_user.has_profile:
            message = {
                "details": "You need to complete your details to send invitations!"
            }
            return Response(message, status=status.HTTP_400_BAD_REQUEST)
